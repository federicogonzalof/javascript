...
function preguntarGolosinas() {
    let preguntaGolosina = prompt('¿Desea sumar golosinas para su funcion? (si/no)')
    if ( preguntaGolosina == 'No' || preguntaGolosina == 'no' || preguntaGolosina == 'NO') {
        alert("Gracias, disfrute de su función")
    } else if (preguntaGolosina == 'Si' || preguntaGolosina == 'si' || preguntaGolosina == 'SI' ) {
        alert('¡Perfecto!')
    } else {
        alert('Ingrese una opción valida')
        preguntarGolosinas()
...
function golosinas() {
    let total = 0
    let nombreProducto = (prompt(`Tenemos las siguientes opciones: 
        \n 1- Papas
        \n 2- Kitkat
        `))
    if (nombreProducto == "1" || nombreProducto == "Papas" || nombreProducto == "papas" || nombreProducto == "PAPAS") {
        let x1 = golosina1.map(x => x.precio)
        let x2 = golosina1.map(x => x.sabor)
        let x3 = golosina1.map(x => x.cantidad)
        let cantidad = parseInt(prompt("¿Cuantas queres?"))
        if (!(isNaN(cantidad))) {
            alert("El precio es de cada una es $" + x1 + " traen " + x3 + " sabor " + x2)
            let subtotal = x1 * cantidad
            alert(`Abonas: $${subtotal} si estas de acuerdo toca en "Aceptar".`)
        } else {
            alert("No ingresaste un numero")
            golosinas()
        }
    } else if (nombreProducto == "2" || nombreProducto == "KitKat" || nombreProducto == "kitkat" || nombreProducto == "KITKAT") {
        let x1 = golosina2.map(x => x.precio)
        let x2 = golosina2.map(x => x.sabor)
        let x3 = golosina2.map(x => x.cantidad)
        let cantidad = parseInt(prompt("¿Cuantas queres?"))
        if (!(isNaN(cantidad))) {
            alert("El precio es de cada una es $" + x1 + " traen " + x3 + " sabor " + x2)
            let subtotal = x1 * cantidad
